...
hourly_xts <- xts::xts(hourly_df[, 6:ncol(hourly_df)], as.POSIXct(hourly_df$date))
...
relative_humidity_2m \[%\]; surface_pressure \[hPa\]; wind_speed_120m \[m/s\]; temperature_120m \[°C\]

## Forecasted Generated Power
```{r}
#| label: plots
#| echo: false
#| fig-width: 11
...
pl_power <- ggplot(hourly_df, aes(x=date, y=p_out)) +
  ...
  labs(y="[kW]", x="UTC", caption = "Forecast Horizon: 168 hours")

pl_power

```

## Daily Forecasted Energy Forecasted Generated Power
...
daily_df <- 
    daily_xts |>
    as_tibble() |>
    mutate(datetime = time(daily_xts)) |>
    mutate(MWh = V1/1000) |>
    relocate(datetime) |>
    slice(2:(n()-1)) 
...
ggplot(daily_df, aes(x=datetime, y=MWh)) +
  ...
  labs(y = "[MWh]", x="Days Ahead", caption = "Forecast Horizon: 6 days ahead") +
  geom_text(aes(label = round(MWh, 2)), vjust = -0.5) +
  scale_x_datetime(date_breaks = "1 day", date_minor_breaks = "1 day", expand = c(0, 0), date_labels = "%d.%m.%Y")
